...
public class VendasController {
    ...
    @FXML private TableColumn<ItemNota, Double> colQtdNota;
    @FXML private TableColumn<ItemNota, String> colPrecoNota;
    @FXML private TableColumn<ItemNota, String> colSubtotalNota;

    @FXML private TableView<Nota> tabelaHistorico;
    @FXML private TableColumn<Nota, String> colDataHistorico;
    @FXML private TableColumn<Nota, String> colTotalHistorico;
    @FXML private TableColumn<Nota, Void> colAcoesHistorico;

    private ObservableList<Nota> obsHistorico = FXCollections.observableArrayList();
    ...
    private void handleAdicionarNota(ActionEvent event) {
        ...
        String qtdTexto = txtQuantidade.getText().trim().replace(",", ".");
        ...
        try {
            double quantidadeInformada = Double.parseDouble(qtdTexto);
            ...
            String formaDeVenda = produtoEncontrado.getTipo().getFormaVenda();
            if (formaDeVenda != null && formaDeVenda.equalsIgnoreCase("Unidade")) {
                if (quantidadeInformada % 1 != 0) { // Se o resto da divisão por 1 não for 0, é decimal
                    exibirAlerta(AlertType.WARNING, "Quantidade Inválida", "Este produto é vendido por UNIDADE.\nNão é possível vender frações (ex: 1.5).");
                    return;
                }
            }

            // verificação do estoque do carrinho atual
            double qtdJaNoCarrinho = 0;
            ...
            exibirAlerta(AlertType.ERROR, "Erro de Formatação", "A quantidade deve ser um número válido. Ex: 2 ou 1.5");
        }
    }
    ...
    private void atualizarResumoEInterface() {
        ...
        if (qtdTotalProdutos % 1 == 0) {
            lblQtdTotal.setText(String.format("%.0f", qtdTotalProdutos));
        } else {
        ...
        // troca de cor baseada nos itens
    ...
    private void configurarTabelaHistorico() {
        ...
        colDataHistorico.setCellValueFactory(cellData -> {
            LocalDateTime data = cellData.getValue().getDataVenda();
            return new SimpleStringProperty(data != null ? data.format(formatter) : "Sem data");
        });

        colTotalHistorico.setCellValueFactory(cellData ->
                new SimpleStringProperty(String.format("R$ %.2f", cellData.getValue().getValorTotal()))
        );

        // "Ver Detalhes"
        colAcoesHistorico.setCellFactory(param -> new TableCell<Nota, Void>() {
            private final Button btn = new Button("Detalhes");
            {
                ...
                btn.setOnAction(event -> {
                    ...
                    exibirAlerta(AlertType.INFORMATION, "Resumo da Venda",
                            "ID Venda: " + nota.getId() + "\n" +
                                    "Total: R$ " + nota.getValorTotal() + "\n" +
    ...
    private void atualizarHistorico() {
        try {
            ...
            obsHistorico.clear();
            obsHistorico.setAll(dao.listarHistorico("VENDA")); // O método que criaremos no DAO
            tabelaHistorico.setItems(obsHistorico);
        } catch (Exception e) {
    ...
    private void carregarPerfilUsuario() {
        ...
        if (usuarioLogado != null) {
            ...
            // ajusta as cores para gerente ou funcionario
